...
export async function generateStaticParams() {
  const slugs = await getAllNewsSlugs("en");
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  try {
    const article = await getNewsArticle("en", slug);
    return {
...
export default async function NewsArticleEnPage({ params }: PageProps) {
  const { slug } = await params;
  const article = await getNewsArticle("en", slug).catch(() => null);
