...
export async function generateStaticParams() {
  const slugs = await getAllNewsSlugs("fr");
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  try {
    const article = await getNewsArticle("fr", slug);
    return {
...
export default async function NewsArticleFrPage({ params }: PageProps) {
  const { slug } = await params;
  const article = await getNewsArticle("fr", slug).catch(() => null);
